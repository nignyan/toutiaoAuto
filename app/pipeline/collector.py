...
def _build_default_pool() -> list[RawSignal]:
    ...
    return [
        ...
        _sig("四川雅安突发山火 火线绵延数公里", 95, [_video(Clarity.CLEAR, 85)]),
        _sig("雅安山火最新进展：救援力量连夜扑救", 88),
        _sig("雅安山火明火已扑灭 转入清理阶段", 70),
        # 组2：台风事件，2 条变体，纯图集（规则2）
        _sig("台风梅花登陆浙江沿海", 92, [_image(), _image()]),
        _sig("台风梅花减弱为热带风暴", 66),
        # 组3：地震事件，2 条变体，视频不足单独立片+单图（规则3）
        _sig("西藏那曲发生4.8级地震", 77, [_video(Clarity.NORMAL, 55), _image()]),
        _sig("那曲地震震感明显 暂无人员伤亡", 61),
        # 组4：无素材信号（规则4 → 素材等待队列）
